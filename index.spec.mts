...
test(
  'Maven Prepare GitHub Action ensures the Maven Wrapper has executable permissions',
  {tags: ['@since-v0.1.0']},
  async () => {
    // ACT
    await importAction();
...
test(
  'Maven Prepare GitHub Action reads the Maven project coordinates from the pom.xml file and exposes them as action outputs',
  {tags: ['@since-v0.1.0']},
  async () => {
    // ARRANGE
    ...
    (getExecOutput as unknown as Mock).mockImplementation((cmd: string, args: string[]) => {
      if (
        cmd === './mvnw' &&
        JSON.stringify(args) ===
          JSON.stringify(['help:evaluate', '-Dexpression=project.groupId', '-q', '-DforceStdout'])
      ) {
      ...
      if (
        cmd === './mvnw' &&
        JSON.stringify(args) ===
          JSON.stringify(['help:evaluate', '-Dexpression=project.version', '-q', '-DforceStdout'])
      ) {
    ...
    expect(core.info).toHaveBeenCalledWith(
      chalk.cyanBright('Maven artifact Major Version:  ') + chalk.greenBright('1'),
    );
    expect(core.info).toHaveBeenCalledWith(
      chalk.cyanBright('Maven artifact Minor Version:  ') + chalk.greenBright('2'),
    );
    expect(core.info).toHaveBeenCalledWith(
      chalk.cyanBright('Maven artifact Patch Version:  ') + chalk.greenBright('3'),
    );
    expect(core.info).toHaveBeenCalledWith(
...
test('Maven Prepare GitHub Action fails when exec throws an error', {tags: ['@since-v0.1.0']}, async () => {
  // ARRANGE
...
test(
  'Maven Prepare GitHub Action does not fail when the Maven artifact id matches the GitHub repository name',
  {tags: ['@since-v0.1.0']},
  async () => {
    // ARRANGE
...
test(
  'Maven Prepare GitHub Action fails when the Maven artifact id does not match the GitHub repository name',
  {tags: ['@since-v0.1.0']},
  async () => {
    // ARRANGE
...
test(
  'Maven Prepare GitHub Action reads git metadata and exposes them as action outputs',
  {tags: ['@since-v0.1.0']},
  async () => {
    // ARRANGE
...
test(
  'Maven Prepare GitHub Action sets git-is-main-branch output to true when current branch matches main-branch input',
  {tags: ['@since-v0.1.0']},
  async () => {
    // ARRANGE
    const repositoryName = github.context.repo.repo.split('/').pop() || '';
...
test(
  'Maven Prepare GitHub Action sets git-is-main-branch output to false when current branch differs from main-branch input',
  {tags: ['@since-v0.1.0']},
  async () => {
    // ARRANGE
    const repositoryName = github.context.repo.repo.split('/').pop() || '';
...
test(
  'Maven Prepare GitHub Action sets maven-artifact-publish to false when a release artifact is already published',
  {tags: ['@since-v0.1.0']},
  async () => {
    // ARRANGE
    const repositoryName = github.context.repo.repo.split('/').pop() || '';
...
test(
  'Maven Prepare GitHub Action sets maven-artifact-publish to true when release artifact lookup reports missing artifact',
  {tags: ['@since-v0.1.0']},
  async () => {
    // ARRANGE
    const repositoryName = github.context.repo.repo.split('/').pop() || '';
...
test.each([
  ...
  'Maven Prepare GitHub Action sets maven-artifact-publish to true when $scenario',
  {tags: ['@since-v0.1.0']},
  async ({stderr}) => {
    // ARRANGE
    const repositoryName = github.context.repo.repo.split('/').pop() || '';
...
test(
  'Maven Prepare GitHub Action fails fast when release artifact lookup fails for non-missing-artifact reasons',
  {tags: ['@since-v0.1.0']},
  async () => {
    // ARRANGE
    const repositoryName = github.context.repo.repo.split('/').pop() || '';
...
test(
  'Maven Prepare GitHub Action includes <empty> marker when release artifact lookup fails without output',
  {tags: ['@since-v0.1.0']},
  async () => {
    // ARRANGE
    const repositoryName = github.context.repo.repo.split('/').pop() || '';
...
test(
  'Maven Prepare GitHub Action sets git-is-latest-main-branch-commit to true when HEAD equals main branch tip',
  {tags: ['@since-v0.1.0']},
  async () => {
    // ARRANGE
    const repositoryName = github.context.repo.repo.split('/').pop() || '';
...
test(
  'Maven Prepare GitHub Action sets docker-tags for snapshot main-branch head',
  {tags: ['@since-v0.1.0']},
  async () => {
    // ARRANGE
    const repositoryName = github.context.repo.repo.split('/').pop() || '';
...
test(
  'Maven Prepare GitHub Action sets docker-tags for release main-branch head',
  {tags: ['@since-v0.1.0']},
  async () => {
    // ARRANGE
    const repositoryName = github.context.repo.repo.split('/').pop() || '';
    ...
    mockExecOutputResponses({
      ...
      [responseKey('./mvnw', [
        'dependency:get',
        `-Dartifact=com.example:${repositoryName}:${mavenVersion}`,
        '--quiet',
      ])]: {
        stdout: '',
        stderr:
...
test(
  'Maven Prepare GitHub Action sets only fixed snapshot and sha tags when commit is not latest main branch head',
  {tags: ['@since-v0.1.0']},
  async () => {
    // ARRANGE
    const repositoryName = github.context.repo.repo.split('/').pop() || '';
...
test(
  'Maven Prepare GitHub Action sets git-is-latest-main-branch-commit to false when all rev-parse candidates fail',
  {tags: ['@since-v0.1.0']},
  async () => {
...
test(
  'Maven Prepare GitHub Action sets github repository name to empty string when repo has no slash',
  {tags: ['@since-v0.1.0']},
  async () => {
...
test('Maven Prepare GitHub Action fails with a non-Error thrown value', {tags: ['@since-v0.1.0']}, async () => {
...
test(
  'Maven Prepare GitHub Action writes all outputs to the GitHub step summary',
  {tags: ['@since-v0.1.0']},
  async () => {
    // ARRANGE
    const repositoryName = github.context.repo.repo.split('/').pop() || '';
